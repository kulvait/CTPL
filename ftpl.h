/*********************************************************
 *
 *  Copyright (C) 2014 by Vitaliy Vitsentiy, modified by Vojtěch Kulvait
 *
...
class thread_pool {
	...
	// the destructor waits for all the functions in the queue to be
	// finished
	...
	// interleave, also with this->stop() nThreads must be >= 0
	void resize(int nThreads) {
		if (!this->isStop && !this->isDone) {
			int oldNThreads =
			    static_cast<int>(this->threads.size());
			if (oldNThreads <= nThreads) {  // if the number of
				...
				for (int i = oldNThreads - 1; i >= nThreads;
				     --i) {
					*this->flags[i] =
					    true;  // this thread will finish
					this->threads[i]->detach();
				}
				{
					// stop the detached threads that were
					// waiting
					std::unique_lock<std::mutex> lock(
					    this->mutex);
					this->cv.notify_all();
				}
				this->threads.resize(nThreads);  // safe to
								 // delete
								 // because the
								 // threads are
								 // detached
				this->flags.resize(nThreads);  // safe to delete
							       // because the
							       // threads have
							       // copies of
							       // shared_ptr of
	...
	void stop(bool isWait = false) {
		if (!isWait) {
			...
			for (int i = 0, n = this->size(); i < n; ++i) {
				*this->flags[i] =
				    true;  // command the threads to stop
			}
			this->clear_queue();  // empty the queue
		} else {
			if (this->isDone || this->isStop) return;
			this->isDone = true;  // give the waiting threads a
					      // command to finish
		}
		{
			std::unique_lock<std::mutex> lock(this->mutex);
		...
		// queue, the functors are not deleted by the threads therefore
		// delete them here
		this->clear_queue();
		this->threads.clear();
		this->flags.clear();
	...
	auto push(F &&f, Rest &&... rest)
		...
		auto pck = std::make_shared<
		    std::packaged_task<decltype(f(0, rest...))(int)>>(
	...
	// thread. returned value is templatized operator returns std::future,
	// where the user can get the result and rethrow the catched exceptins
	template <typename F>
	auto push(F &&f) -> std::future<decltype(f(0))> {
		auto pck =
		    std::make_shared<std::packaged_task<decltype(f(0))(int)>>(
			std::forward<F>(f));
		auto _f = new std::function<void(int id)>(
		    [pck](int id) { (*pck)(id); });
		this->q.push(_f);
	...
	void set_thread(int i) {
		...
		auto f = [this, i,
			...
			while (true) {
				while (isPop) {  // if there is anything in the
						 // queue
					std::unique_ptr<
					    std::function<void(int id)>>
					    func(_f);  // at return, delete the
						       // function even if an
						       // exception occurred
					(*_f)(i);
					if (_flag)
						return;  // the thread is wanted
							 // to stop, return even
							 // if the queue is not
							 // empty yet
					else
				...
				// the queue is empty here, wait for the next
				// command
				std::unique_lock<std::mutex> lock(this->mutex);
				++this->nWaiting;
				...
				--this->nWaiting;
				if (!isPop)
					return;  // if the queue is empty and
						 // this->isDone == true or
						 // *flag then return
			}
		};
		this->threads[i].reset(new std::thread(
		    f));  // compiler may not support std::make_unique()
	}
